...
bl_info = {
    "name": "Surface Follow",
    "author": "Rich Colburn, email: the3dadvantage@gmail.com, vitos1k(2.8 ported)",
    "version": (1, 1),
    "blender": (2, 80, 0),
...
def triangulate(ob='empty', proxy=False):
    ...
    if proxy:
        ...
        mods = False
    depsgraph = bpy.context.evaluated_depsgraph_get()
    proxy_eval = ob.evaluated_get(depsgraph)
    mesh_from_eval = proxy_eval.to_mesh()
    obm = get_bmesh(mesh_from_eval)   
    bmesh.ops.triangulate(obm, faces=obm.faces)
    obm.to_mesh(mesh_from_eval)
    count = len(mesh_from_eval.polygons)    
    tri_idx = np.zeros(count * 3, dtype=np.int64)        
    mesh_from_eval.polygons.foreach_get('vertices', tri_idx)        
...
def multi_bind():
    ...
    if obj == None:
        return -1
    olist = [i for i in bpy.context.selected_objects if i.type == 'MESH']
    count = len(olist)
...
def remove_handler(type):
    ...
    if type == 'scene':
        ...
        if run_handler in bpy.app.handlers.depsgraph_update_pre:
            #bpy.app.handlers.scene_update_pre.remove(run_handler)
            bpy.app.handlers.depsgraph_update_pre.remove(run_handler)
    if type == 'frame':
...
def add_handler(type):
    ...
    if type == 'scene':        
        ...
        bpy.app.handlers.depsgraph_update_pre.append(run_handler)
    if type == 'frame':        
...
class SCENE_PT_SurfaceFollowPanel(bpy.types.Panel):
    """Surface Follow Panel"""
    bl_label = "Surface Follow Panel"
    bl_idname = "SCENE_PT_SurfacePanel"
    ...
    def draw(self, context):
        ...
        col.operator("scene.surface_update_once", text="Update Once", icon='XRAY')        
...
# Register Clases -------------->>>
classes = (SCENE_PT_SurfaceFollowPanel,BindToSurface,UpdateOnce,ToggleSurfaceFollow)
