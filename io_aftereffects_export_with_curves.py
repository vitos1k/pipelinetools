...
bl_info = {
	"name": "Export: Adobe After Effects with Curves(.jsx)",
	"description": "Export cameras, selected objects & camera solution "
		"3D Markers to Adobe After Effects CS3 and above",
	"author": "Bartek Skorupa, (Adi Samsonoff), vitos1k",
	"version": (0, 0, 67),
	"blender": (2, 80, 0),
	"location": "File > Export > Adobe After Effects with Curves (.jsx)",
	"warning": "",
...
def get_selected(context):
	...
	curves = [] # list of all selected curves that can be exported as AE's solid masks
	...
	for ob in obs:
		if ob.type == 'CAMERA':
			...
			curves.append([ob, ob.type + convert_name(ob.name)])       # Type of curves added to name
		else:
...
def Normalize_coords(co_x,co_y,width,height,resolution_percentage):
	render_scale = resolution_percentage / 100
	render_size = (int(width * render_scale),int(height * render_scale),)
	#x = round(co_x * render_size[0])
	#y = round(co_y * render_size[1])
	x = co_x * float(render_size[0])
	y = co_y * float(render_size[1])
	return (x,y)
...
def world_to_camera_normalized(coord,obj,width,height,resolution_percentage):
	curr_scene = bpy.context.scene
	co_local = obj.matrix_world.normalized().inverted() @ coord
	z = -co_local.z

	camera = obj.data
	frame = [-v for v in camera.view_frame(scene=curr_scene)[:3]]
	...
	y = 1- y
	x,y = Normalize_coords(x,y, width, height, resolution_percentage)
	return Vector((x, y, z))
...
def offset_handles(co2d,in_coords,out_coords):
	#there goes crazy constructions 

	#inco2d[0] = in_coords[0]-co2d[0]
	#inco2d[1] = in_coords[1]-co2d[1]
	#outco2d[0] = out_coords[0] - co2d[0]
	#outco2d[1] = out_coords[1] - co2d[1]
	mainpoint_vector = Vector((co2d[0],co2d[1]))
	in_vector = Vector((in_coords[0],in_coords[1]))-mainpoint_vector
	out_vector = Vector((out_coords[0],out_coords[1]))-mainpoint_vector        
	distance_in = in_vector.length
	distance_out = out_vector.length
	if (distance_in+distance_out)!=0:
		between_vector = (out_vector-in_vector)*(distance_in/(distance_in+distance_out))+in_vector
		in_vector -= between_vector
		out_vector -= between_vector
	else:
		in_vector = Vector((0,0))
		out_vector = Vector((0,0))   
	#______________________________________________________________________________________________________________________________________________
	return in_vector,out_vector

# jsx script for AE creation
def write_jsx_file(file, data, selection, include_animation, include_active_cam, include_selected_cams, include_selected_objects, include_cam_bundles,point_resolution,subdivide_edges):
	...
	# create structure for curves
	for i, obj in enumerate(selection['curves']):
		if include_selected_objects:
			name_ae = selection['curves'][i][1]
			js_data['curves'][name_ae] = {
				'type': selection['curves'][i][0].type,
				'position': '',
				'position_static': '',
				'position_anim': False,
				'closed': False,
	...
	for frame in range(data['start'], end):
		...
		if include_selected_objects:
			...
			for i, ob in enumerate(selection['curves']):
				name_ae = selection['curves'][i][1]
				ob_mw = ob[0].matrix_world
				camera_object = data['active_cam_frames'][0]
				for j,spline in enumerate(ob[0].data.splines):
					points = ''
					inpoints = ''
					outpoints = ''
					num_points = point_resolution

					if spline.type == 'BEZIER':
						bpoints = spline.bezier_points
					else:
						bpoints = spline.points             


					bez_len = len(bpoints)
					bez_range = list(range(1,bez_len,1))
					lastopointo = bez_len - 1
					if spline.use_cyclic_u:
						bez_range.append(0)
						js_data['curves'][name_ae]['closed'] = True
					else:
						js_data['curves'][name_ae]['closed'] = False
					...
					if subdivide_edges:
						for i in bez_range:
							...
							p2 = bpoints[i-1]
							if spline.type == 'BEZIER':
								#dots = interpolate_bezier(ob_mw @ p1.co,ob_mw @ p1.handle_right,ob_mw @ p2.handle_left,ob_mw @ p2.co,num_points)
								dots = interpolate_bezier(ob_mw @ p2.co,ob_mw @ p2.handle_right,ob_mw @ p1.handle_left,ob_mw @ p1.co,num_points)
							else:
								dots = interpolate_bezier(ob_mw @ p2.co,ob_mw @ p2.co,ob_mw @ p1.co,ob_mw @ p1.co,num_points)
							
							if i!=lastopointo:
								dots.pop()
							for co in  dots:
								co2d = world_to_camera_normalized(co,camera_object, data['width'], data['height'], data['resolution_percentage'])                               
								points+='[%f,%f],' % (co2d[0],co2d[1])
								inpoints+='[0.0,0.0],'
								outpoints+='[0.0,0.0],'
					else:
						inco2d = Vector((0,0))
						outco2d = Vector((0,0))
						for p1 in bpoints:
							co2d = world_to_camera_normalized(ob_mw @ p1.co,camera_object,  data['width'], data['height'], data['resolution_percentage'])                         
							if spline.type == 'BEZIER':                                
								in_coords = world_to_camera_normalized(ob_mw @ p1.handle_left, camera_object, data['width'], data['height'], data['resolution_percentage'])
								out_coords = world_to_camera_normalized(ob_mw @ p1.handle_right, camera_object, data['width'], data['height'], data['resolution_percentage'])
								inco2d,outco2d = offset_handles(co2d,in_coords,out_coords)                              
							else:
								inco2d = Vector((0,0))
								outco2d = Vector((0,0))
							points +='[%f,%f],' % (co2d[0],co2d[1])
							inpoints +='[%f,%f],' % (inco2d[0],inco2d[1])
							outpoints +='[%f,%f],' % (outco2d[0],outco2d[1])


					points = '[%s],' % (points)
					inpoints = '[%s],' % (inpoints)
					outpoints = '[%s],' % (outpoints)
					if frame == data['start']:
						js_data['curves'][name_ae]['splines_points'].append(points)
						js_data['curves'][name_ae]['splines_in'].append(inpoints)
						js_data['curves'][name_ae]['splines_out'].append(outpoints)
					else:
						js_data['curves'][name_ae]['splines_points'][j]+=points
						js_data['curves'][name_ae]['splines_in'][j]+=inpoints
						js_data['curves'][name_ae]['splines_out'][j]+=outpoints
	...
	jsx_file.write('var timesarray = [%s];\n' % (js_data['times']))
	...
	# create 2D solids with path (masks)
	jsx_file.write('// **************  CURVES  **************\n\n\n')
	for i, obj in enumerate(js_data['curves']):
		name_ae = obj        
		jsx_file.write('var %s = newComp.layers.addSolid([0.0,0.0,0.0],"%s", %i, %i,%f,%f);\n' % (name_ae,name_ae,data['width'],data['height'],data['aspect'],data['duration']))
		jsx_file.write('%s.threeDLayer = false;\n' % name_ae)
		#jsx_file.write('%s.source.name = "%s";\n' % (name_ae, name_ae))
		solidpos = '[%f,%f]' % (data['width']*data['resolution_percentage']/200,data['height']*data['resolution_percentage']/200)
		jsx_file.write('%s.property("position").setValue(%s);\n' % (name_ae,solidpos))

		# Set values of properties, add kyeframes only where needed
		jsx_file.write('myShape = new Shape();\n')
		if js_data['curves'][name_ae]['closed']:
			jsx_file.write('myShape.closed = true;\n')
		else:
			jsx_file.write('myShape.closed = false;\n')
		for j,spline_pts in enumerate(js_data['curves'][name_ae]['splines_points']):
			jsx_file.write('newMask = %s.Masks.addProperty("Mask");\n' % (name_ae))
			jsx_file.write('newMask.name = "spline_%i";\n' % (j))
			jsx_file.write('myMaskShape = newMask.property("maskShape");\n')
			jsx_file.write('var %s%i_vertices = [%s];\n' % (name_ae,j,spline_pts))
			jsx_file.write('var %s%i_intangents = [%s];\n' % (name_ae,j,js_data['curves'][name_ae]['splines_in'][j]))
			jsx_file.write('var %s%i_outtangents = [%s];\n\n\n' % (name_ae,j,js_data['curves'][name_ae]['splines_out'][j]))

			
			jsx_file.write('for (i=0;i<%s%i_vertices.length;i++){\n' % (name_ae,j))
			jsx_file.write('myShape.vertices = %s%i_vertices[i];\n' %(name_ae,j))
			jsx_file.write('myShape.inTangents = %s%i_intangents[i];\n' % (name_ae,j))
			jsx_file.write('myShape.outTangents = %s%i_outtangents[i];\n' % (name_ae,j))
			jsx_file.write('myMaskShape.setValueAtTime(timesarray[i],myShape);}\n')
...
def main(file, context, include_animation, include_active_cam, include_selected_cams, include_selected_objects, include_cam_bundles,point_resolution,subdivide_edges):
	data = get_comp_data(context)
	selection = get_selected(context)
	write_jsx_file(file, data, selection, include_animation, include_active_cam, include_selected_cams, include_selected_objects, include_cam_bundles,point_resolution,subdivide_edges)
...
class ExportJsx(bpy.types.Operator, ExportHelper):
	...
	bl_label = "Export to Adobe After Effects including Curves"
	filename_ext = ".jsx"
	...
	subdivide_edges: BoolProperty(
			name="Subdivide curves",
			description="Subdivide every edge in curves, to better match with original, when left/right handles doesn't align in blender/AE",
			default=False,
			)
	point_resolution: IntProperty(
			name="Subdivision resolution",
			description="how much subdivisions 1 = None, 2 = Subdivide once, etc",
			default=1,
			)
	...
	def draw(self, context):
		...
		box.label(text="Curves parameters:")
		box.prop(self, 'subdivide_edges')
		box.prop(self, 'point_resolution')        
	...
	def execute(self, context):
		return main(self.filepath, context, self.include_animation, self.include_active_cam, self.include_selected_cams, self.include_selected_objects, self.include_cam_bundles,self.point_resolution,self.subdivide_edges)


def menu_func(self, context):
	self.layout.operator(ExportJsx.bl_idname, text="Adobe After Effects with Curves(.jsx)")
